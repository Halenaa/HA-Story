...
class SimpleDiversityAnalyzer:
    ...
    def simple_bleu_score(self, candidate_tokens: List[str], reference_tokens_list: List[List[str]]) -> float:
        """简化的BLEU分数计算"""
        if not candidate_tokens or not reference_tokens_list:
            return 0.0
        
        # 计算1-gram和2-gram的精确度
        candidate_1grams = candidate_tokens
        candidate_2grams = [tuple(candidate_tokens[i:i+2]) for i in range(len(candidate_tokens)-1)]
        
        # 计算1-gram精确度
        matches_1gram = 0
        for ref_tokens in reference_tokens_list:
            ref_1grams = ref_tokens
            matches_1gram += len(set(candidate_1grams) & set(ref_1grams))
        
        precision_1gram = matches_1gram / (len(candidate_1grams) * len(reference_tokens_list)) if candidate_1grams else 0
        
        # 计算2-gram精确度
        matches_2gram = 0
        for ref_tokens in reference_tokens_list:
            ref_2grams = [tuple(ref_tokens[i:i+2]) for i in range(len(ref_tokens)-1)]
            matches_2gram += len(set(candidate_2grams) & set(ref_2grams))
        
        precision_2gram = matches_2gram / (len(candidate_2grams) * len(reference_tokens_list)) if candidate_2grams else 0
        
        # 简化的BLEU分数 (几何平均)
        if precision_1gram > 0 and precision_2gram > 0:
            bleu_score = (precision_1gram * precision_2gram) ** 0.5
        else:
            bleu_score = 0.0
    ...
    def calculate_self_bleu(self, texts: List[str]) -> float:
        ...
        for i in range(3):
            # 当前文本作为候选
            candidate_tokens = self.simple_tokenize(texts[i])
            
            # 其他两篇作为参考
            reference_tokens_list = [self.simple_tokenize(texts[j]) for j in range(3) if j != i]
            
            # 计算BLEU分数
            bleu_score = self.simple_bleu_score(candidate_tokens, reference_tokens_list)
    ...
    def learn_alpha_weights(self, group_results: Dict[str, Dict]) -> Dict[str, Dict]:
        ...
        for key, result in group_results.items():
            # 直接从result中获取temperature，因为已经在group_result中设置了
...
def main():
    parser = argparse.ArgumentParser(description='简化版内容多样性分析')
    ...
    analyzer = SimpleDiversityAnalyzer(args.data_dir)
