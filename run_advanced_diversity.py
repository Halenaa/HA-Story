...
def main():
    """主函数"""
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='高级内容多样性分析系统')
    ...
    try:
        ...
        if group_results:
            ...
            for key, result in group_results.items():
                print(f"  {key[0]}_{key[1]}_T{key[2]}: diversity_score={result['diversity_score']:.4f}, alpha={result['alpha']:.4f}")
